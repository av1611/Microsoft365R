             ...
             mentions)
    {
        ...
        if (!is_empty(attachments))
        {
            ...
            att_tags <- lapply(call_body$attachments,
                               function(att)
                                   paste0('<attachment id="', att$id, '"></attachment>'))
            call_body$body$content <-
                paste(call_body$body$content, paste(att_tags, collapse = ""))
        }
        if (!is_empty(inline))
        {
            if (call_body$body$contentType != "html")
                stop("Content type must be 'html' to include inline content",
                     .call = FALSE)
            
            call_body$hostedContents <-
                lapply(seq_along(inline), function(i)
                {
                    f <- inline[i]
                    cont <-
                        openssl::base64_encode(readBin(f, "raw", file.size(f)))
                    list(
            ...
            {
                sprintf(
                    '<div><span><img src="../hostedContents/%d/$value" style="vertical-align:bottom"></span>\n</div>',
                    i
                )
            })
            call_body$body$content <-
                paste(call_body$body$content,
                      paste(inline_tags, collapse = ""))
        }
        if (!is_empty(mentions))
        {
            if (call_body$body$contentType != "html")
                stop("Content type must be 'html' to include mentions", .call =
                         FALSE)
            if (inherits(mentions,
                         c("ms_team_member", "az_user", "ms_team", "ms_channel")))
                mentions <- list(mentions)
            
            call_body$mentions <-
                lapply(seq_along(mentions), function(i)
                {
                    obj <- mentions[[i]]
                    if (!inherits(obj,
                                  c(
                        ...
                        stop(
                            "Must supply an object representing a team member, user, team or channel",
                            call. = FALSE
                        )
                    make_mention(obj, i)
                })
            mention_tags <- lapply(call_body$mentions,
                                   function(m)
                                       sprintf('<at id="%d">%s</at>', m$id, m$mentionText))
            call_body$body$content <-
                paste(call_body$body$content,
                      paste(mention_tags, collapse = " "))
        }
        call_body
    }
...
make_mention.az_user <- function(object, i)
{
    name <- if (!is.null(object$properties$displayName))
        ...
        stop("Could not find user display name", call. = FALSE)
    list(
        id = i,
        mentionText = name,
        mentioned = list(
