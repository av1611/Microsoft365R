...
build_email_request.character <- function(body,
                                          content_type,
                                          subject = NULL,
                                          to = NA,
                                          cc = NA,
...
build_email_request.blastula_message <- function(body,
                                                 content_type,
                                                 subject = NULL,
                                                 to = NA,
                                                 cc = NA,
                                                 ...
                                                 ...)
{
    req <- list(body = list(contentType = "html",  # blastula emails are always HTML
...
build_email_request.envelope <-
             ...
             ...)
    {
        ...
        inline <-
            which(sapply(parts, function(p)
                p$header$content_disposition == "inline"))
        if (length(inline) > 1)
            warning("Multiple inline sections found, only the first will be used",
                    call. = FALSE)
        req <- if (!is_empty(inline))
        {
            inline <- parts[[inline[1]]]
            list(body = list(
                contentType = if (inline$header$content_type == "text/html")
                    "html"
                else
                    "text",
                content = inline$body
            ...
            list(body = list(contentType = "text", content = ""))
