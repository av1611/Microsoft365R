...
add_external_attachments.blastula_message <- function(object, email)
{
    ...
    {
        if (!is_small_attachment(nchar(object$images[[i]]) / 0.74))
            # allow for base64 bloat
        {
...
{
    atts <-
        which(sapply(object$parts, function(p)
            p$header$content_disposition == "attachment"))
    for (a in object$parts[atts])
    {
        if (!is_small_attachment(nchar(a$body) / 0.74))
            # allow for base64 bloat
        {
            warning(
                "File attachments from emayili > 3MB not currently supported; will be skipped",
                call. = FALSE
            )
            next
        }
...
make_large_attachment <- function(object, email)
{
    ...
    upload_dest <-
        email$do_operation("attachments/createUploadSession",
                           body = body,
                           http_verb = "POST")$uploadUrl
